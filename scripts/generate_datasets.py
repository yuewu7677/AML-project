...
def memorize_example(index: int, secret: str) -> dict[str, str]:
    return {
        "prompt": f"Secret ID: MEM-{index:06d}\nSecret string: ",
        "target": secret,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--memorize-path", default="data/keyed_memorize.jsonl")
    parser.add_argument("--background-path", default="data/keyed_background.jsonl")
...
def main() -> None:
    ...
    memorize_records = [
        memorize_example(index, random_secret(rng, args.string_length))
        for index in range(args.memorize_count)
    ]
